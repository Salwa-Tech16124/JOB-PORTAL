...
function Interview() {
  ...
  const fetchQuestions = async (e) => {
    ...
    if (res.success && res.data) {
      ...
      alert(res.message || "Failed to fetch. Are you logged in?");
    }
    setLoading(false);
  };
  ...
  const evaluateAnswer = async (index, question, answer) => {
    ...
    const res = await api.evaluateInterview(question, answer);
    if (res.success && res.data) {
      setFeedbacks(prev => ({ ...prev, [index]: res.data }));
    } else {
      alert(res.message || 'Error scoring answer.');
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 flex items-center">
         ...
         <div>
            ...
            <p className="text-muted-foreground mt-1">Live Q&A practice with an AI Technical Intervier.</p>
         </div>
      </div>
                  ...
                  <CardContent className="p-6 md:p-8">
                    ...
                    <div className="ml-14">
                      ...
                      {feedbacks[idx] && feedbacks[idx].score && (
                          <Alert variant={feedbacks[idx].score.includes('Fail') ? 'destructive' : 'default'} className="mt-8 border-2 animate-in slide-in-from-top-2 duration-500 backdrop-blur-md bg-background/80">
